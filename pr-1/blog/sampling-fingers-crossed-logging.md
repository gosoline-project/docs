# Log Sampling in Go: Less Noise, More Debuggability
...
## Why traditional log sampling isn’t enough[​](#why-traditional-log-sampling-isnt-enough "Direct link to Why traditional log sampling isn’t enough")
...
## The two ideas[​](#the-two-ideas "Direct link to The two ideas")

### 1) Sampling is a decision carried in context[​](#1-sampling-is-a-decision-carried-in-context "Direct link to 1) Sampling is a decision carried in context")
...
### 2) Fingers-crossed logging buffers context and only emits on failure[​](#2-fingers-crossed-logging-buffers-context-and-only-emits-on-failure "Direct link to 2) Fingers-crossed logging buffers context and only emits on failure")
...
## The behavior model (simple mental map)[​](#the-behavior-model-simple-mental-map "Direct link to The behavior model (simple mental map)")
...
If it’s sampled, you log as usual. If it’s not sampled, you buffer; if it fails, you flush.

***

## How to enable it in a gosoline service[​](#how-to-enable-it-in-a-gosoline-service "Direct link to How to enable it in a gosoline service")

### How to configure sampling[​](#how-to-configure-sampling "Direct link to How to configure sampling")
...
### How to enable sampling in the application[​](#how-to-enable-sampling-in-the-application "Direct link to How to enable sampling in the application")
...
### Strategy guide (how strategies decide)[​](#strategy-guide-how-strategies-decide "Direct link to Strategy guide (how strategies decide)")

#### `tracing`: follow upstream trace sampling (X-Ray / OpenTelemetry)[​](#tracing-follow-upstream-trace-sampling-x-ray--opentelemetry "Direct link to tracing-follow-upstream-trace-sampling-x-ray--opentelemetry")
...
#### `probabilistic`: guaranteed baseline + extra traffic[​](#probabilistic-guaranteed-baseline--extra-traffic "Direct link to probabilistic-guaranteed-baseline--extra-traffic")
...
### multiple strategies[​](#multiple-strategies "Direct link to multiple strategies")
...
### How the sampling decision is forwarded downstream in stream messages[​](#how-the-sampling-decision-is-forwarded-downstream-in-stream-messages "Direct link to How the sampling decision is forwarded downstream in stream messages")
...
## How fingers-crossed works in HTTP servers and stream consumers[​](#how-fingers-crossed-works-in-http-servers-and-stream-consumers "Direct link to How fingers-crossed works in HTTP servers and stream consumers")
...
### HTTP server behavior[​](#http-server-behavior "Direct link to HTTP server behavior")
...
### Stream consumer behavior[​](#stream-consumer-behavior "Direct link to Stream consumer behavior")
...
## How to use “fingers-crossed” logging manually (jobs, CLIs, workers)[​](#how-to-use-fingers-crossed-logging-manually-jobs-clis-workers "Direct link to How to use “fingers-crossed” logging manually (jobs, CLIs, workers)")
...
## How to do HTTP request sampling (including a “force sample” override)[​](#how-to-do-http-request-sampling-including-a-force-sample-override "Direct link to How to do HTTP request sampling (including a “force sample” override)")
...
## Rollout guidance (practical tips)[​](#rollout-guidance-practical-tips "Direct link to Rollout guidance (practical tips)")
...
## Closing: why this is worth it[​](#closing-why-this-is-worth-it "Direct link to Closing: why this is worth it")
...
* [logging](/docs/pr-1/blog/tags/logging.md)
* [sampling](/docs/pr-1/blog/tags/sampling.md)
* [observability](/docs/pr-1/blog/tags/observability.md)
