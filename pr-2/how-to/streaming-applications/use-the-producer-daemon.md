...
Kafka does not support Gosoline aggregation in the producer daemon. Kafka's own producer performs batching and compression, and its `max_batch_size` and `max_batch_bytes` settings override daemon batch limits. See [general Kafka usage](/docs/pr-2/how-to/kafka/general/.md#producer-daemon-usage).
