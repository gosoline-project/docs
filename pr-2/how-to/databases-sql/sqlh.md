...
The HTTP server configuration is described in the httpserver documentation. The database configuration follows the same format described in the [sqlc documentation](/docs/pr-2/how-to/databases-sql/sqlc/.md#configuration).
...
Entities use the same `sqlr.Entity` base struct described in the [sqlr documentation](/docs/pr-2/how-to/databases-sql/sqlr/.md#defining-entities):
...
The filter is converted to an `sqlc.Expression` and applied as a WHERE condition on the query. See the [sqlc JSON filter documentation](/docs/pr-2/how-to/databases-sql/sqlc/.md) for the full filter syntax.
...
`sqlh` tags are additive to the association behavior already defined by `sqlr`. The relationship itself must still be declared via an explicit `sqlr` relationship tag or discovered through [`sqlr` auto-detected relationships](/docs/pr-2/how-to/databases-sql/sqlr/.md#auto-detected-relationships); the `sqlh` tag only adds CRUD-specific preload and sync behavior on top. `db:"-"` remains optional for relationship fields. In particular, `sqlh` preload phases are merged with any preload behavior already configured in `sqlr`, rather than replacing it.

For creates, `sqlh:"preload:create"` is applied to the underlying `sqlr.Create()` builder, so the entity returned by the create handler is reloaded with those relations already hydrated. This follows the same post-create preload behavior described in [`sqlr` Create with Post-Create Preloading](/docs/pr-2/how-to/databases-sql/sqlr/.md#create-with-post-create-preloading).

For updates, `sqlh:"preload:update"` is applied to the underlying `sqlr.Update()` builder, so the entity returned by the update handler is reloaded with those relations already hydrated. This follows the same post-update preload behavior described in [`sqlr` Update with Post-Update Preloading](/docs/pr-2/how-to/databases-sql/sqlr/.md#update-with-post-update-preloading).

For delete operations, `sync:delete` configures the association cleanup passed to the underlying `sqlr.Delete()` call, following the same cleanup semantics described in [`sqlr` Delete with Association Cleanup](/docs/pr-2/how-to/databases-sql/sqlr/.md#delete-with-association-cleanup).

Tags are only valid on association fields recognised by [`sqlr` relationships](/docs/pr-2/how-to/databases-sql/sqlr/.md#relationships); using them on scalar fields causes startup to fail with an error. Tagged associations are traversed recursively, so nested paths such as `Child.Nested` are picked up automatically when the related entity also declares `sqlh` tags.

For the underlying `sqlr` behavior behind these options, see [Read with Association Loading](/docs/pr-2/how-to/databases-sql/sqlr/.md#read-with-association-loading), [Eager Loading with Preload](/docs/pr-2/how-to/databases-sql/sqlr/.md#eager-loading-with-preload), [Create with Selective Association Persistence](/docs/pr-2/how-to/databases-sql/sqlr/.md#create-with-selective-association-persistence), [Create with Post-Create Preloading](/docs/pr-2/how-to/databases-sql/sqlr/.md#create-with-post-create-preloading), [Update with Association Sync](/docs/pr-2/how-to/databases-sql/sqlr/.md#update-with-association-sync), and [Delete with Association Cleanup](/docs/pr-2/how-to/databases-sql/sqlr/.md#delete-with-association-cleanup).
