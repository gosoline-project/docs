...
## Default server[​](#default-server "Direct link to Default server")
...
## Multiple named servers[​](#multiple-named-servers "Direct link to Multiple named servers")
...
| `concurrency`    | ConcurrencySettings | -           | Concurrent request and connection pressure limits. See [Concurrency and connection pressure](/docs/pr-4/how-to/http-server/concurrency-and-connection-pressure/.md). |
...
### Compression exclude settings[​](#compression-exclude-settings "Direct link to Compression exclude settings")
...
Always exclude SSE endpoints from compression. Gzip buffering will break real-time streaming. See [Stream with SSE](/docs/pr-4/how-to/http-server/stream-with-sse/.md) for details.
...
### Error settings[​](#error-settings "Direct link to Error settings")
...
With the default `private` setting, internal 5xx errors return `{"err":"internal server error"}`. Errors below 500 wrapped with `NewErrorWithStatus` still expose their message to API clients.
...
### Logging settings[​](#logging-settings "Direct link to Logging settings")
...
### Router settings[​](#router-settings "Direct link to Router settings")
...
### Timeout settings[​](#timeout-settings "Direct link to Timeout settings")
...
## Graceful shutdown[​](#graceful-shutdown "Direct link to Graceful shutdown")
...
For full details on connection lifecycle, connection pressure management, and concurrent request limiting, see [Concurrency and connection pressure](/docs/pr-4/how-to/http-server/concurrency-and-connection-pressure/.md).
