...
## sqlc - SQL Client[​](#sqlc---sql-client "Direct link to sqlc - SQL Client")
...
## sqlr - SQL Repository[​](#sqlr---sql-repository "Direct link to sqlr - SQL Repository")
...
* Generic Repository pattern with Create, Read, Update, Delete, Query, and Count operations
...
* Row locking with `ForUpdate()` inside transactions
* Transaction-aware repository variant

## sqlh - SQL HTTP Handlers[​](#sqlh---sql-http-handlers "Direct link to sqlh - SQL HTTP Handlers")

The `sqlh` package provides typed, transaction-aware CRUD handlers for SQLR entities. It integrates with the gosoline HTTP server.

**When to use sqlh:**

* Building REST APIs for entity management
* Need CRUD endpoints without repetitive transaction and repository code
* Need JSON Merge Patch, server-owned filters, or custom identity and delete behavior

**Key capabilities:**

* Typed CRUD definitions with create, update, patch, list, and delete operations
* Automatic transactions with commit and rollback behavior
* JSON Merge Patch with selective association synchronization
* Typed list results with shared query and count scopes
* Force filters for authorization and tenant restrictions
* Integration with gosoline HTTP server routing

## Choosing the Right Layer[​](#choosing-the-right-layer "Direct link to Choosing the Right Layer")
