...
```
go get github.com/gosoline-project/sqlc@v0.4.0
```
...
```
query := sqlc.From("experiments").

    Where(sqlc.Col("status").Eq("scheduled")).
...
// sql: SELECT * FROM `experiments` WHERE `status` = ? LIMIT ? OFFSET ? FOR UPDATE

// args: []any{"scheduled", 10, 20}
```

The generic builder exposes the same method:

```
query := sqlc.FromG[Experiment]("experiments").
...
* SQLH creates one transaction for each default CRUD operation and uses a transaction-aware SQLR repository built from the same SQL client.
* For custom SQLH operations, use `sqlh.TxRunner` or `sqlc.Client.WithTx()` and pass the active transaction to SQLR or SQLC operations.
* SQLR `Count()` strips row locking and pagination. Apply the same filters and scope to the list query and count query.
