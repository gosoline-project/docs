...
The `sqlr` package provides a generic, type-safe repository layer built on top of [`sqlc`](/docs/pr-4/how-to/databases-sql/sqlc/.md). It offers CRUD operations, relationship management, eager loading via joins and preloads, and transaction support — all using Go generics for compile-time type safety.
...
The `sqlr` package uses `sqlc` under the hood for database connections. Configure your database using the same `sqlc` configuration key described in the [sqlc documentation](/docs/pr-4/how-to/databases-sql/sqlc/.md#configuration):
...
| `Count(ctx, qb)`               | Counts the rows selected by a query builder. It preserves joins, filters, grouping, and `HAVING`, but ignores ordering, pagination, and row locking. Pass `nil` to count all rows.                                                                                                |
...
err := client.WithTx(ctx, func(tx sqlc.Tx) error {
    ...
    transaction := sqlr.NewTx(tx)

    entities, err := repositoryTx.Query(transaction, func(qb *sqlr.QueryBuilderSelect) {

        qb.Where(sqlc.Col("status").Eq("scheduled")).
...
## Counting Query Results[​](#counting-query-results "Direct link to Counting Query Results")

Use `Count()` when an endpoint needs a total beside a paginated result set:
...
`Count()` preserves joins, `WHERE`, `GROUP BY`, and `HAVING`. It removes `ORDER BY`, `LIMIT`, `OFFSET`, and `FOR UPDATE` before it builds the count query. A grouped query counts the rows in its grouped result. Pass `nil` to count all rows.

Use `RepositoryTx.Count(ttx, qb)` when the count must use an active transaction. SQLH uses the same filter and delete scope for its list query and count operation.
...
For more details about gosoline fixture loading, grouping, purge behavior, and named fixture references, see the [fixtures package reference](/docs/pr-4/reference/package-fixtures/.md).
