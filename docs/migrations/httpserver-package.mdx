...
This guide explains how to migrate from the old `github.com/justtrackio/gosoline/pkg/httpserver` package directly to the standalone `github.com/gosoline-project/httpserver` module at `v0.4.0`.
...
```bash
go get github.com/gosoline-project/httpserver@v0.4.0
```
...
In `v0.4.0`, the default error handler exposes 4xx error messages but sanitizes 5xx responses as `{"err":"internal server error"}`. Binding and validation failures from `Bind` and `BindSse` are client errors and return `400 Bad Request`.
...
| Error responses | The default handler exposes 4xx error messages and returns `{"err":"internal server error"}` for 5xx responses. |
| CORS | `api_cors_allowed_origin_pattern` is matched against the full `Origin` value. Partial regex matches do not allow origins. |
...
- Review `max_body_bytes`; keep the v0.4.0 default 10 MiB limit, raise it, or set it to `0` intentionally.
- Update tests for v0.4.0 behavior: binding errors return 400, default 5xx bodies are sanitized, CORS patterns match full origins, health responses hide module error details, and profiling binds to loopback.
