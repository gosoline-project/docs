...
The entry point is `cli.NewCli()`. It embeds a `*Router`, which is a tree of named groups and commands. When you call `Run()`, it parses `os.Args`, resolves the matching command, injects parsed flags into the gosoline config, and starts the kernel with the command's module.
...
Register a `Cmd` directly on the `Cli` (or on any child `*Router` returned by `Group()`). Each `Cmd` needs a `Name` and a `ModuleFactory` — the function that builds the module to run:
...
`cli.WithRunFunc` is a convenience wrapper that turns a plain function into a `kernel.ModuleFactory`.
...
apiRouter.Cmd(cli.Cmd{Name: "serve", ModuleFactory: /* ... */})

dbRouter := c.Group(cli.Group{Name: "db"})
dbRouter.Cmd(cli.Cmd{Name: "migrate", ModuleFactory: /* ... */})
```
...
Read the value inside your module factory:

```go
ModuleFactory: cli.WithRunFunc(func(ctx context.Context, config cfg.Config, logger log.Logger) (kernel.ModuleRunFunc, error) {
    port := config.GetString("httpserver.default.port") // from CfgKey
