...
```bash
go get github.com/gosoline-project/sqlh@v0.6.0
```
...
| Handler | Flow |
|---------|------|
| `HandleCreate` | Binds `IC`, calls `TransformCreateInput`, persists via `repo.Create()`, then renders the created entity |
...
|-----------|--------|--------|
| `preload` | `read`, `query`, `update` | Adds `Preload()` to the matching CRUD builder; on `update`, it configures the post-update reload used for the returned entity |
