...
```bash
go get github.com/gosoline-project/sqlh@v0.4.0
```
...
type Author struct {
    ...
    Profile   Profile       `db:"-,belongsTo:profile_id" sqlh:"preload:read,query,update"`
    Tags      []Tag         `db:"-,many2many:author_tags" sqlh:"preload:read;sync:create,update"`
}
```
...
| `sync` | `create`, `update` | Calls `SyncAssociation()` for the association while persisting the entity |

`sqlh` tags are additive to the association behavior already defined by `sqlr`. The relationship itself must still be declared via the `db` tag or discovered through [`sqlr` auto-detected relationships](./sqlr.mdx#auto-detected-relationships); the `sqlh` tag only adds CRUD-specific preload and sync behavior on top. In particular, `sqlh` preload phases are merged with any preload behavior already configured in `sqlr`, rather than replacing it.
