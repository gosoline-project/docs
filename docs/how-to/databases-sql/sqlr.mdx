...
```bash
go get github.com/gosoline-project/sqlr@v0.6.0
```
...
`sqlr` uses dot-separated association paths such as `Posts.Comments` anywhere an API needs to identify relationships on an entity graph. That includes association persistence on `Create()`, association synchronization on `Update()`, association cleanup on `Delete()`, and relation loading through `Preload()` on `Read()` and `Query()`.
...
- `Preload()` on `Read()` and `Query()` accepts direct and nested relation paths.
