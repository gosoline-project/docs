...
| `Update(ctx, entity, opts...)` | Updates only the base entity row by default; associations remain untouched unless explicitly synchronized |
...
If you want `Update()` to also insert, update, delete, or unlink related records, opt in per call with `QueryBuilderUpdate`:
...
5. **ManyToMany** - listed related entities are inserted or updated as needed; missing join-table links are removed and new ones are inserted.
...
If the parent row does not exist, `Update()` with association synchronization enabled returns `ErrNotFound` and rolls back the transaction.
...
The `sqlr.ErrNotFound` sentinel error is returned when `Read()` or `Delete()` cannot find the requested entity, and when `Update()` with association synchronization enabled cannot find the parent row:
