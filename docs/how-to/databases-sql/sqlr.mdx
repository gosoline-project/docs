...
```bash
go get github.com/gosoline-project/sqlr@v0.5.0
```
...
| `CreatedAt` | `db:"created_at,autoCreateTime"` | Set automatically on insert by default |
| `UpdatedAt` | `db:"updated_at,autoUpdateTime"` | Set automatically on insert and update by default |
...
Column mappings and behavior are controlled via the `db` struct tag. The `db` tag is **optional** for public fields — when omitted, the field name is converted to a column name using [`SchemaNameTransformer`](#schemanametransformer) (default: snake_case). Public struct and slice-of-struct fields without a `db` tag can also be auto-detected as relationships when `sqlr` finds matching entity and foreign-key evidence (see [Auto-detected Relationships](#auto-detected-relationships)). To explicitly exclude a public field from all mapping, use `db:"-"` (without any options after the comma).

| Tag | Description |
|-----|-------------|
| `db:"column_name"` | Maps the field to a database column |
| `db:"column_name,primaryKey"` | Marks the field as the primary key |
| `db:"column_name,autoCreateTime"` | Auto-sets the field to `time.Now()` on insert by default |
| `db:"column_name,autoUpdateTime"` | Auto-sets the field to `time.Now()` on insert and update by default |
...
| `db:"-,...,preload"` | Automatically preloads the relationship on every `Read()` and `Query()` call |
...
Relationships can be declared in two ways: via **auto-detection** of eligible untagged public fields (convention over configuration) or via **explicit `db` struct tags** (full control over every name).
...
| Method | Description |
|--------|-------------|
| `Create(ctx, entity, opts...)` | Inserts the entity and any eligible populated association fields; optional create builder methods can limit or omit association paths |
...
| `Update(ctx, entity, opts...)` | Updates only the base entity row by default; associations remain untouched unless explicitly synchronized, and many-to-many updates stay link-only unless `SyncManyToManyEntities()` is used |
...
- Selecting a nested path also traverses the required ancestors, so `Posts.Comments` persists both `Posts` and their `Comments`.
...
Modify the entity and pass it to `Update()`. By default, only the base row is updated and the `UpdatedAt` timestamp is refreshed automatically. Any populated association fields on the entity are ignored unless association sync is explicitly enabled for that call:
...
If you want `Update()` to also insert, update, delete, or unlink related records, opt in per call with `QueryBuilderUpdate`. Existing many-to-many related rows are a special case: by default, `Update()` only reconciles join-table membership for them and does not update their columns unless you explicitly opt that path into full entity synchronization.
...
Opt selected many-to-many paths into full related-row synchronization with `SyncManyToManyEntities(paths...)`:

```go
updated, err := repo.Update(ctx, &article, func(qb *sqlr.QueryBuilderUpdate) {
    qb.SyncAllAssociations()
    qb.SyncManyToManyEntities("Tags")
})
```
...
5. **ManyToMany** - related entities with zero PKs are inserted; existing related rows are link-synchronized by default, so missing join-table links are removed and new ones are inserted without updating the related rows themselves unless that path was opted into `SyncManyToManyEntities()`.
...
Use association paths to control which relations are synchronized during `Update()`. Paths can point to direct relations or nested relations within the entity graph you pass to `Update()`.

- Paths use dot notation for nested relations, such as `Posts.Comments`.
- Selecting a nested path also traverses the required ancestors, so `Posts.Comments` synchronizes both `Posts` and their `Comments`.
- `SyncManyToManyEntities(paths...)` only accepts paths that resolve to many-to-many relations and enables full recursive synchronization for those paths.
- Omitting a parent path such as `Posts` also omits all nested descendants.
- Invalid association paths return an error.
...
`Delete()` removes only the base entity row.
...
The `sqlr.ErrNotFound` sentinel error is returned when `Read()` or `Delete()` cannot find the requested entity, when `Update()` with association synchronization enabled cannot find the parent row, and when default many-to-many synchronization references a related entity by primary key that does not exist:
