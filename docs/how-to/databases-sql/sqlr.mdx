...
```bash
go get github.com/gosoline-project/sqlr
```
...
| Method | Description |
|--------|-------------|
| `Create(ctx, entity)` | Inserts the entity, sets auto-increment ID and timestamps |
...
```go
qb.LeftJoin("Posts", sqlr.Condition("status = ?", "published"))
```
