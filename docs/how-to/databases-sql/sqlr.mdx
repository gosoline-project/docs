...
```bash
go get github.com/gosoline-project/sqlr@v0.7.0
```
...
| `sqlr:"...;preload"` | Automatically preloads the relationship on every `Read()` and `Query()` call, and on `Update()` whenever `sqlr` reloads the entity after persistence |
...
| Method | Description |
|--------|-------------|
| `Create(ctx, entity, opts...)` | Inserts the entity and any eligible populated association fields; optional create builder methods can limit, omit, or disable association persistence for a call |
...
`sqlr` uses dot-separated association paths such as `Posts.Comments` anywhere an API needs to identify relationships on an entity graph. That includes association persistence on `Create()`, association synchronization on `Update()`, association cleanup on `Delete()`, and relation loading through `Preload()` on `Read()`, `Query()`, and post-update reloads on `Update()`.
...
- `Preload()` on `Read()`, `Query()`, and `Update()` post-reloads accepts direct and nested relation paths.
...
Use `Preload()` to load related entities in separate queries. On `Read()` and `Query()`, preloads run as part of the lookup; on `Update()`, preloads run in a follow-up reload after the update succeeds. Preloads support all relationship types: HasOne, HasMany, BelongsTo, and ManyToMany.
...
Add the `preload` tag option to a relationship to automatically load it on every `Read()` and `Query()` call, and on `Update()` whenever `sqlr` performs a post-update reload — without requiring an explicit `Preload()` call:
...
Auto-preloads are recursively discovered across nested relationships and merged with any explicit preloads, including `QueryBuilderUpdate.Preload()` requests (explicit preloads take precedence when both are present).
