...
Within the `sqlr` tag, options are separated with semicolons. The only comma-separated form is the grouped sync option `sync:create,update`.
...
| `sqlr:"...;preload"` | Automatically preloads the relationship on every `Read()` and `Query()` call |
...
| `sqlr:"...;sync:create,update"` | Synchronizes the relationship by default during both `Create()` and `Update()` |
...
| `Update(ctx, entity, opts...)` | Updates only the base entity row by default; associations remain untouched unless explicitly synchronized per call or via `sync:update` tags, and many-to-many updates stay link-only unless `SyncMany2many()` is used |
...
- `SyncMany2many(paths...)` only accepts paths that resolve to many-to-many relations and enables full recursive synchronization for those paths.
