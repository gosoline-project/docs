...
Entities with a non-zero primary key are treated as already-persisted: they are skipped for insertion but the FK or join table row is still created. `Update` only affects the base entity row by default, but it can synchronize all or selected associations when explicitly enabled. `Delete` only removes the base entity row.
