...
```bash
go get github.com/gosoline-project/httpserver@v0.2.1
```
...
The `binding` tag uses [go-playground/validator](https://pkg.go.dev/github.com/go-playground/validator/v10). If validation fails, the request is rejected with a descriptive error:
...
Return `nil, error` for unexpected errors. The error middleware returns a `500 Internal Server Error` with `{"err": "<message>"}`:
