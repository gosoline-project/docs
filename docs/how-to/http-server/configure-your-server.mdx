...
| `concurrency` | ConcurrencySettings | - | Concurrent request and connection pressure limits. See [Concurrency and connection pressure](/how-to/http-server/concurrency-and-connection-pressure/). |
...
:::caution
Always exclude SSE endpoints from compression. Gzip buffering will break real-time streaming. See [Stream with SSE](/how-to/http-server/stream-with-sse/) for details.
:::
...
For full details on connection lifecycle, connection pressure management, and concurrent request limiting, see [Concurrency and connection pressure](/how-to/http-server/concurrency-and-connection-pressure/).
