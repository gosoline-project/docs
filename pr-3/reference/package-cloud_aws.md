...
## The cloud/aws package[​](#the-cloudaws-package "Direct link to The cloud/aws package")
...
### General service config[​](#general-service-config "Direct link to General service config")
...
| ----------------------------------------------- | --------------------------------------------------------------------------------- | ---------------------------- |
| assume\_role                                    | If defined, this role will be used to gain access                                 | ""                           |
| backoff.cancel\_delay                           | If the request get canceled, how long should the cancel delayed                   | 1s                           |
| backoff.initial\_interval                       | The initial duration to wait before retrying the request on error                 | 50ms                         |
| backoff.max\_attempts                           | How many attempts should be done                                                  | 10 (0 means retry forever)   |
| backoff.max\_interval                           | Max duration between 2 calls when retrying                                        | 10s                          |
| backoff.max\_elapsed\_time                      | For how long the service should retry the request                                 | 10m (0m means retry forever) |
| credentials\_cache.expiry\_window               | The duration before cached credentials expire                                     | 5m                           |
| credentials\_cache.expiry\_window\_jitter\_frac | The fraction of the expiry window to use as jitter                                | 0.1                          |
| endpoint                                        | Which service endpoint should be called                                           | <http://localhost:4566>      |
| http\_client.timeout                            | After what duration the request should be canceled, if the server doesn't respond | 0s (no timeout)              |
| profile                                         | If defined, this aws profile will be used to fetch credentials                    | ""                           |
| region                                          | The region in use                                                                 | eu-central-1                 |

### Cloudwatch[​](#cloudwatch "Direct link to Cloudwatch")
...
### Different Client Names[​](#different-client-names "Direct link to Different Client Names")

You can specify different client names to customize specific fields for a particular client while inheriting the rest of the configuration. For example:
      ...
      clients:
        ...
        someClientName:
          ...
          naming:
            ...
            table_pattern: "{app.namespace}-{name}"
```
      ...
      clients:
        ...
        clientWithoutEnvInName:
          ...
          naming:
            ...
            table_pattern: "{app.tags.group}-{name}"
      ...
      clients:
        ...
        clientWithoutEnvInName:
          ...
          naming:
            ...
            queue_pattern: "{app.tags.group}-{queueId}"
```
...
* **`naming.table_pattern`**: `{app.tags.group}-{name}` (from `cloud.aws.dynamodb.clients.clientWithoutEnvInName`).
...
* **`naming.queue_pattern`**: `{app.tags.group}-{queueId}` (from `cloud.aws.sqs.clients.clientWithoutEnvInName`).
* **`credentials`**: `default-access-key` and `default-secret` (from `cloud.aws.defaults`).
* **`backoff.type`**: `infinite` (from `cloud.aws.defaults.clientWithoutEnvInName`).
